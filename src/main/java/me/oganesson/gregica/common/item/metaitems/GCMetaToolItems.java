...
public class GCMetaToolItems {
    ...
    public static void init() {
        Choocher = register(gregtech.api.items.toolitem.ItemGTTool.Builder.of(Gregica.MOD_ID, "choocher").toolStats(b -> b.blockBreaking().crafting().damagePerCraftingAction(2).attackDamage(1.0F).attackSpeed(-2.8F).behaviors(new IToolBehavior[]{new EntityDamageBehavior(2.0F, new Class[]{EntityGolem.class}), BlockRotatingBehavior.INSTANCE})).oreDict(ToolOreDict.toolHammer).secondaryOreDicts(ToolOreDict.toolWrench).sound(SoundEvents.BLOCK_ANVIL_LAND).symbol('t').toolClasses(new String[]{"hammer", "wrench"}));
    }
}
