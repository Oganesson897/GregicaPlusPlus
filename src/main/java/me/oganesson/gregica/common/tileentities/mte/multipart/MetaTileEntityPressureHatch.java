...
public class MetaTileEntityPressureHatch extends MetaTileEntityMultiblockPart implements IMultiblockAbilityPart<IPressureContainer>, IDataInfoProvider {
    ...
    public void update() {
        ...
        if (!getWorld().isRemote && getOffsetTimer() % 20 == 0) {
            ...
            boolean needsPressureIncrease = pressureContainer.getPressure() > pressureContainer.getMinPressure() && this.pressureContainer.getMinPressure() < GCValues.EARTH_PRESSURE;
            // pressure container, needs to decrease pressure
            boolean needsPressureDecrease = pressureContainer.getPressure() < pressureContainer.getMaxPressure() && this.pressureContainer.getMaxPressure() > GCValues.EARTH_PRESSURE;
            ...
            if (canChangePressure) {
                ...
                if (te != null) {
                    IPressureContainer container = te.getCapability(GCCapabilities.CAPABILITY_PRESSURE_CONTAINER, this.getFrontFacing().getOpposite());
                    if (container != null) {
    ...
    public MultiblockAbility<IPressureContainer> getAbility() {
        return GCCapabilities.PRESSURE_CONTAINER;
    }
