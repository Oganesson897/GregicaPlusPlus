...
import me.oganesson.gregica.api.recipe.recipeproperties.PressureProperty;
...
public class PressureMultiblockRecipeLogic extends MultiblockRecipeLogic {
    ...
    private double recipePressure = GCValues.EARTH_PRESSURE;
    ...
    protected void updateRecipeProgress() {
        ...
        } else if (this.recipeEUt > 0 || this.recipePressure != GCValues.EARTH_PRESSURE) {
    ...
    protected boolean drawPressure(double pressure, boolean simulate) {
        ...
        if (pressure != GCValues.EARTH_PRESSURE) pressureToChange = containerPressure * 0.01;
        else return true;

        if (pressure > GCValues.EARTH_PRESSURE) pressureToChange = -pressureToChange;
    ...
    protected void setupRecipe(Recipe recipe) {
        ...
        if (recipe.getRecipePropertyStorage() != null && recipe.hasProperty(PressureProperty.getInstance())) {
            this.recipePressure = recipe.getProperty(PressureProperty.getInstance(), GCValues.EARTH_PRESSURE);
        }
    }
    ...
    public void deserializeNBT(@Nonnull NBTTagCompound compound) {
        ...
        if (this.progressTime > 0) {
            ...
            this.recipePressure = GCValues.EARTH_PRESSURE;
        }
    }
}
