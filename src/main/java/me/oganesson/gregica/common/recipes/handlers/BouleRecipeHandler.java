package me.oganesson.gregica.common.recipes.handlers;
...
public class BouleRecipeHandler {
    ...
    public static void processCrystallizer(OrePrefix gem, @Nonnull Material material, GemProperty property) {
        ...
        if (material.getMaterialComponents().size() > GCRecipeMaps.CRYSTALLIZER_RECIPES.getMaxInputs() - 1 + GCRecipeMaps.CRYSTALLIZER_RECIPES.getMaxFluidInputs()) {
            return;
        }
        ...
        for (MaterialStack materialStack : material.getMaterialComponents()) {
            ...
            if (componentMaterial.isSolid() || componentMaterial.hasProperty(PropertyKey.DUST)) {
                ...
                if (inputs.size() > GCRecipeMaps.CRYSTALLIZER_RECIPES.getMaxInputs() - 1) {
                    return;
                }
                ...
                componentAmount += amount;
                if (fluidInputs.size() > GCRecipeMaps.CRYSTALLIZER_RECIPES.getMaxFluidInputs()) {
                    return;
                }
        ...
        BlastRecipeBuilder builder = GCRecipeMaps.CRYSTALLIZER_RECIPES.recipeBuilder()
