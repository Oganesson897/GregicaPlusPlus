package me.oganesson.gregica.common.tileentities.mte.multi.pressure;
...
public class MetaTileEntityTurbomolecularPump extends MultiblockWithDisplayBase implements IWorkable {
        ...
        if (this.variantActiveBlocks != null) {
            for (BlockPos blockPos : this.variantActiveBlocks) {
                ...
                if (blockState.getBlock() instanceof VariantActiveBlock) {
                    this.getWorld().setBlockState(blockPos, blockState.withProperty(BlockFireboxCasing.ACTIVE_DEPRECATED, isActive));
                }
            }
        }
    }
    ...
    protected BlockPattern createStructurePattern() {
        return FactoryBlockPattern.start(RelativeDirection.RIGHT, RelativeDirection.FRONT, RelativeDirection.DOWN)
                ...
                .where('P', states(getCasingState())
                        .or(abilities(GCCapabilities.PRESSURE_CONTAINER).setExactLimit(1)))
                .build();
    }
    ...
    protected void updateLogic() {
        ...
        IPressureContainer container = getAbilities(GCCapabilities.PRESSURE_CONTAINER).get(0);
        if (container.getPressure() == targetPressure || targetPressure < GCValues.EARTH_PRESSURE) return; // do nothing when at max
        ...
        } else if (container.getMaxPressure() < GCValues.EARTH_PRESSURE) {
            if (drainTanks(true)) {
    ...
    protected ModularUI.Builder createUITemplate(EntityPlayer entityPlayer) {
        ...
        if (this.isStructureFormed()) {
            ...
            builder.widget(new TextFieldWidget2(12, 108, 112, 16, () -> String.valueOf(targetPressure), value -> {
                if (!value.isEmpty()) targetPressure = Math.min(GCValues.EARTH_PRESSURE, Math.max(Double.parseDouble(value), minPressure));
    ...
    public void addInformation(ItemStack stack, @Nullable World player, List<String> tooltip, boolean advanced) {
        ...
        tooltip.add(I18n.format("gregica.multiblock.turbomolecular_pump.tooltip.1",
                NumberFormattingUtil.formatDoubleToCompactString(this.pressureRate),
                NumberFormattingUtil.formatDoubleToCompactString(this.minPressure), I18n.format(GCValues.PNF[GCUtil.getTierByPressure(this.minPressure)])));
        tooltip.add(I18n.format("gregica.multiblock.turbomolecular_pump.tooltip.2", FLUID_USE_AMOUNT));
        tooltip.add(I18n.format("gregica.multiblock.turbomolecular_pump.tooltip.3", I18n.format(GTValues.VNF[this.tier])));
    }
    ...
    protected ICubeRenderer getFrontOverlay() {
        if (tier == GTValues.LuV) {
            return GCTextures.HIGH_POWER_TURBOMOLECULAR_PUMP;
        }

        return GCTextures.LOW_POWER_TURBOMOLECULAR_PUMP;
    }
    ...
    public static Function<String, String> getTextFieldValidator() {
        return val -> {
            if (val.isEmpty()) return "" + GCValues.EARTH_PRESSURE;
            return val;
        };
    }
}
