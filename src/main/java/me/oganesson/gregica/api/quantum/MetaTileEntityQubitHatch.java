...
public class MetaTileEntityQubitHatch extends MetaTileEntityMultiblockPart implements IMultiblockAbilityPart<IQubitContainer> {
    ...
    public MetaTileEntityQubitHatch(ResourceLocation metaTileEntityId, int tier, int parallel, boolean isExportHatch) {
        ...
        if (isExportHatch) {
            this.qubitContainer = QubitContainerHandler.emitterContainer(this, (long) GCPPValues.QUBIT[tier] * parallel, GCPPValues.QUBIT[tier], parallel);
        } else {
            this.qubitContainer = QubitContainerHandler.receiverContainer(this, (long) GCPPValues.QUBIT[tier] * parallel, GCPPValues.QUBIT[tier], parallel);
        }
    }
    ...
    public MultiblockAbility<IQubitContainer> getAbility() {
        return isExportHatch ? GCPPCapabilities.OUTPUT_QBIT : GCPPCapabilities.INPUT_QBIT;
    }
