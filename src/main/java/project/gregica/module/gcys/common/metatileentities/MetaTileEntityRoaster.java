package project.gregica.module.gcys.common.metatileentities;
...
import project.gregica.api.recipe.recipeproperties.NoCoilTemperatureProperty;
...
public class MetaTileEntityRoaster extends RecipeMapMultiblockController implements IHeatingCoil {
    ...
    public MetaTileEntityRoaster(ResourceLocation metaTileEntityId) {
        super(metaTileEntityId, GCRecipeMaps.ROASTER_RECIPES);
    }
        ...
        for (int x = -1; x <= 1; x++) {
            for (int z = -1; z <= 1; z++) {
                ...
                if (blockState.getBlock() instanceof BlockFireboxCasing) {
                    blockState = blockState.withProperty(BlockFireboxCasing.ACTIVE_DEPRECATED, isActive);
    ...
    public void addInformation(ItemStack stack, @Nullable World player, List<String> tooltip, boolean advanced) {
        ...
        if (Keyboard.isKeyDown(Keyboard.KEY_LSHIFT)) {
            tooltip.add(I18n.format("gregica.multiblock.roaster.tooltip.1"));
            tooltip.add(I18n.format("gregica.multiblock.roaster.tooltip.2"));
            tooltip.add(I18n.format("gregica.multiblock.roaster.tooltip.3"));
            tooltip.add(I18n.format("gregica.multiblock.roaster.tooltip.4"));
        } else {
    ...
    protected ICubeRenderer getFrontOverlay() {
        return GCTextures.ROASTER_OVERLAY;
    }
