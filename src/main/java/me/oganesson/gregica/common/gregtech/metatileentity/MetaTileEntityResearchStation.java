...
import me.oganesson.gregica.api.quantum.QubitConsumeRecipeLogic;
...
public class MetaTileEntityResearchStation extends QubitRecipeMapMultiblockController implements IWorkable {
    public MetaTileEntityResearchStation(ResourceLocation metaTileEntityId) {
        super(metaTileEntityId, GCPPRecipeMaps.RESEARCH_STATION);
    ...
    protected BlockPattern createStructurePattern() {
        return FactoryBlockPattern.start()
                ...
                .where('B', states(getCasingState1()).or(abilities(MultiblockAbility.EXPORT_ITEMS).setMinGlobalLimited(1)).or(abilities(MultiblockAbility.INPUT_ENERGY).setMinGlobalLimited(1)).or(abilities(MultiblockAbility.MAINTENANCE_HATCH).setExactLimit(1)).or(abilities(GCPPCapabilities.INPUT_QBIT).setMinGlobalLimited(1)))
